...
def load_corpus(path) -> [Document]:
    ...
    :return:
    """
    df, corpus_dict = _load_corpus_as_dataframe(path)
    df.apply(_row_to_doc_dict, axis=1)

    return _corpus, corpus_dict
...
def _load_corpus_as_dataframe(path):
    ...
    corpus_dict = {int(key): value for key, value in json_data.items()}
...
def _row_to_doc_dict(row: pd.Series):
    _corpus[row['Id']] = Document(row['Id'], row['Tweet'][0:50], row['Tweet'], row['Date'], row['Likes'],
                                  row['Retweets'],
...
def corpus_to_dict(corpus):
    ...
    corpus_dict = {int(key): value for key, value in corpus.items()}
    return corpus_dict
